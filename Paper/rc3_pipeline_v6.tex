...
\\
\indent The SDSS data is pass through the $\texttt{photo}$ pipeline% (\citet{photopaper})
...
The data is queried using SkyServer through a SDSS Command Line Query Tool. Then the the calibrated, sky-subtracted corrected frame with the calibration meta-data ($\texttt{fpC}$) is retrieved in bulk from the Science Archive Server (SAS). 
	\subsubsection{2MASS}
	 The 2MASS(\citet{2mass}) is a near-infared survey in J(1.25 $\mu$m), H(1.67 $\mu$m), and K$_s$(2.17 $\mu$m) band. Data taken from two 1.3m telescope covered almost the whole sky with  overlapping``survey tile" is 6 degree long by 8.5 arcminute stripes.% for telescope schedulling purposes.

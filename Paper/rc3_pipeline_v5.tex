...
Astronomical catalogs such as the famous Messier Catalog and New General Catalog has a historical usage in helping  astronomers who are studying object of some particular property.
astronomers . This is particularly important in the days when all-sky information was not avaliable, so data was taken by , since
common property.
Astronomical catalog usage in the age of sky surveys
for studies 
...
value is still evident in its usage in literature. Selected galaxies or complete subsets are used in astrophysical studies of  quasars and X-ray sources (CITE) or  galaxy morphology and properties(CITE).  The RC3 catalog is also statistical studies  for cosmology purposes Cross correlated also used in  Updated Catalog, such as the New York University-Value Added Galaxy Catalog (NYU-VAGC)
...
There is an ununiform update of the catalog over the years (-----) after the catalog was publish, more accurate position was collected from whatever sources/data they had access to.
...
\subsection{Data}
	\subsubsection{SDSS}
	\subsection{2MASS}
	 The Two Micron All Sky Survey (2MASS; CITE) is a infared survey in the J,H,K (WRITE repsective wavelengths) bands.
